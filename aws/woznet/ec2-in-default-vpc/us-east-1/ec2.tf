...
// Abandon the use of datascan ec2 module. At least for now.
...
#
#  #iam_instance_profile = aws_iam_instance_profile.ec2_profile.name # added manually until I figure out what it is.
...
resource "aws_instance" "default_bastion" {
  ...
  #availability_zone = ""
  key_name = "Toulon"
  vpc_security_group_ids = [module.default_sg.id]
  subnet_id = aws_subnet.public_subnet[0].id
...
# Do I need this now that I add amos to route53?
#output "public_ip_of_bastion_server" {
#  value = module.default_bastion.module_ec2.public_ip
#}

#output "allem" {
#  value = aws_instance.default_bastion.public_ip
#}
