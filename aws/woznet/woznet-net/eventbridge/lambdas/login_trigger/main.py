...
def lambda_handler(event, context):
  ...
  #message = str(event['detail']['userIdentity']['type']) + " user " + \
  #          str(event['detail']['userIdentity']['userName']) + \
  #          "(" + str(event['detail']['userIdentity']['arn']) + ")" + \
  #          " logged in at " + str(event['detail']['eventTime']) + \
  #          " from " + str(event['detail']['sourceIPAddress'])

  message = str(event['detail']['userIdentity']['type']) + " " + \
            str(event['detail']['userIdentity']['userName']) + " " + \
            "(" + str(event['detail']['userIdentity']['arn']) + ")" + \
            " logged in at " + str(event['detail']['eventTime']) + \
            " from " + str(event['detail']['sourceIPAddress'])
